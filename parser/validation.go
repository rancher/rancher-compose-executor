...
func getValue(val interface{}, context string) string {
	...
	for i, k := range keys {
		...
		case config.RawServiceMap:
			val = typedVal[k]
		case config.RawService:
			val = typedVal[k]
...
func oneOfMessage(serviceMap config.RawServiceMap, schema map[string]interface{}, err, nextErr gojsonschema.ResultError) string {
	switch nextErr.Type() {
...
func validate(serviceMap config.RawServiceMap) error {
...
func validateV2(serviceMap config.RawServiceMap) error {
...
func generateErrorMessages(serviceMap config.RawServiceMap, schema map[string]interface{}, result *gojsonschema.Result) error {
...
func validateServiceConstraints(service config.RawService, serviceName string) error {
	if err := setupSchemaLoaders(schemaDataV1, &schemaV1, &schemaLoaderV1, &constraintSchemaLoaderV1); err != nil {
		return err
	}
